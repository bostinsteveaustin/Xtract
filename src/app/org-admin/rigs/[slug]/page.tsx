...
import { OrgRigVersionActions } from "@/components/org-admin/org-rig-version-actions";
import { NewOrgRigVersionForm } from "@/components/org-admin/new-org-rig-version-form";
import { AttachOrgEvidenceForm } from "@/components/org-admin/attach-org-evidence-form";
...
/**
 * Organisation Rig detail — mirrors /admin/rigs/[slug] but scoped to the
 * active tenant. Version history, state transitions, calibration evidence,
 * and new-version authoring all route through /org-admin/rigs/actions.ts
 * which re-verifies the Rig's organisation against the caller's active org.
 */
export default async function OrgRigDetailPage({ params }: Props) {
  ...
  // Resolve parent lineage for display.
  const { data: parent } = rig.forked_from_rig_id
    ? await admin
        .from("rigs")
        .select("id, slug, name")
        .eq("id", rig.forked_from_rig_id)
        .single()
    : { data: null };
          ...
          {parent && (
            <div className="mt-2 text-xs text-slate-500">
              ...
              {rig.forked_from_version
                ...
                : ""}
              . Calibration evidence did not transfer on fork (E-08 §4.4).
            </div>
          )}
        </div>
      </div>
                  ...
                  <div>
                    ...
                    {evidence.length === 0 ? (
                      <p className="text-xs text-slate-500">
                        No evidence attached. Promotion to{" "}
                        <span className="font-mono">validated</span> requires
                        at least two independent successful domain tests.
                      </p>
                    ) : (
                    ...
                    <div className="mt-3">
                      <AttachOrgEvidenceForm rigVersionId={v.id} />
                    </div>
                  </div>
                </li>
              );
