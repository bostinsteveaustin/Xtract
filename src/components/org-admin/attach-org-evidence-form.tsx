...
import { attachOrgCalibrationEvidence } from "@/app/org-admin/rigs/actions";
...
export function AttachOrgEvidenceForm({
  rigVersionId,
}: {
  rigVersionId: string;
}) {
  ...
  function onSubmit() {
    ...
    startTransition(async () => {
      try {
        await attachOrgCalibrationEvidence(
          rigVersionId,
          evidenceType,
          parsed
        );
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          <Label htmlFor={`org_evidence_type-${rigVersionId}`}>Type</Label>
          <select
            id={`org_evidence_type-${rigVersionId}`}
            value={evidenceType}
      ...
      <div className="space-y-1">
        <Label htmlFor={`org_payload-${rigVersionId}`}>Payload (JSON)</Label>
        <Textarea
          id={`org_payload-${rigVersionId}`}
          value={payloadText}
