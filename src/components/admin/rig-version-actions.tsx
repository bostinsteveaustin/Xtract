...
import { transitionRigVersionState } from "@/app/admin/rigs/actions";

/**
 * Client control for state transitions on a single rig_version. Wraps the
 * server action in an AlertDialog for confirmation — promotion to validated
 * or deprecated is not a reversible action.
 */
export function RigVersionActions({
  rigVersionId,
  currentState,
}: {
  rigVersionId: string;
  ...
  function run(target: "experimental" | "validated" | "deprecated") {
    ...
    startTransition(async () => {
      try {
        await transitionRigVersionState(rigVersionId, target);
      } catch (err) {
